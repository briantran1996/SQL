...
#1. List all VIPs along with the hotel they reserved (if any).
...
#2. List VIPs who reserved a hotel, showing their name and the hotel name.
SELECT v.first_name, v.last_name, h.hotel_name
FROM vips v
INNER JOIN reservations r ON v.member_id = r.member_id
INNER JOIN hotels h ON r.hotel = h.hotel_id;

#3. List VIPs with their event name and the hotel they reserved (if any).
...
#4. List the names of VIPs who have the same reservation hotel as the VIP named 'Amina Al-Hassan'.
...
WHERE r.hotel = (
    ...
    INNER JOIN vips v2 ON r2.member_id = v2.member_id
    WHERE v2.first_name = 'Amina' AND v2.last_name = 'Al-Hassan');

#5. Find pairs of VIPs where one provides feedback to the other. Show their names and relationship.
SELECT v1.first_name || ' ' || v1.last_name AS VIP, 
       v2.first_name || ' ' || v2.last_name AS ProvidesFeedbackTo
FROM vips v1
INNER JOIN vips v2 ON v1.provides_feedback_to = v2.member_id;

#6. Count how many VIPs are associated with each association type.
SELECT assoc_type, COUNT(*) AS vip_count
FROM vips
GROUP BY assoc_type;

#7. List the hotels with the number of VIPs who reserved each hotel.
SELECT h.hotel_name, COUNT(r.member_id) AS vip_reservations
FROM hotels h
